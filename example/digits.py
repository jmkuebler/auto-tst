...
import numpy as np
import autotst
from autotst.model import AutoGluonImagePredictor
...
data_Q = np.array(
    images_sorted[np.random.randint(len(zeros), len(zeros) + len(ones), size=20)]
)


# ======= Run default ========
# need to flatten images for tabular predictor
tst_new = autotst.AutoTST(
    data_P.reshape((len(data_P), -1)), data_Q.reshape((len(data_Q), -1))
)
print(
    "P-value with default settings and tabular predictor (time limit of 1 minutes): ",
    tst_new.p_value(),
)
